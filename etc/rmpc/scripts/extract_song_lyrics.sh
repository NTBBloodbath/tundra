...
#
# The difference between this script and the extract_album_lyrics one is that this one is meant
# to be used as part of my beets hook scripts. That means, this script is ran right after every
# song file metadata write (aka right after adding the lyrics into the song metadata).

if [ $# -ne 2 ]; then
    echo "Usage: $0 \"Artist Album Title\" \"Song path/name\""
    exit 1
fi
